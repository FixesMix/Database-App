...
//Aura 2/11/2023 CPS*3330, CPS*4981 - ADD YOUR information here
WriteLine("It runs!");
...
conn.ConnectionString = "Server = MSI\\SQLEXPRESS; Database = Users; User Id = nierandfar; Password = whereveryouare;";
conn.Open();

// Create the command 
SqlCommand command = new SqlCommand("SELECT * FROM AuraTable", conn);
...
SqlCommand insertCommand = new SqlCommand("INSERT INTO AuraTable (UserId,UserName,  UserEmail) VALUES (@0, @1, @2)", conn);
...
try
{
    ...
    SqlCommand errorCommand = new SqlCommand("SELECT * FROM AuraTable", conn);
